/*
Something wrong with the build system of tsheredoc.

Copy from sourcecode https://github.com/jwalton/tsheredoc/blob/master/src/index.ts
 */
...
function stringToHeredoc(value: string) {
  ...
  return (
    lines.map((line) => line.slice(minIndentLength)).join('\n') +
    (addNewline ? '\n' : '')
  )
}
 ...
 *     `); // val2 == "test\ntest\n"
 */
export default function heredoc(
